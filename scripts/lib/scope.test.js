...
test('feature pode alterar flags e env/nonprod', () => {
  assert.strictEqual(checkScope('feature/x', ['flags/ft_a.json', 'env/nonprod/ft_a.json', 'catalog/home/keys.json']).ok, true);
});
...
test('release pode alterar env/prod, rm e catalog', () => {
  assert.strictEqual(checkScope('release/1.0', ['env/prod/ft_a.json', 'rm/RM-1.json', 'catalog/home/keys.json']).ok, true);
});
...
test('remove só pode apagar FF (flags, env/nonprod, env/prod, rm); catalog pode mudar', () => {
  assert.strictEqual(checkScopeChanges('remove/x', ch('D flags/a.json', 'D env/nonprod/a.json', 'D env/prod/a.json', 'D rm/RM-1.json', 'M catalog/home/keys.json')).ok, true);
});
