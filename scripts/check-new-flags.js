...
const mode = branch.startsWith('feature/') ? 'feature' : branch.startsWith('update/') ? 'update' : branch.startsWith('remove/') ? 'remove' : null;
...
(async () => {
  const { newKeys, changedKeys, removedKeys } = classify(parseNameStatus(sh(`git diff --name-status --no-renames ${base}...HEAD`)));
  ...
  const r = evaluate({ mode, newKeys, changedKeys, removedKeys, repoKeys, remoteKeys });
  if (!r.ok) {
    console.error(`✗ ${branch}: nome de FF duplicado ou branch errada:\n${r.problems.map((p) => `  - ${p}`).join('\n')}`);
    process.exit(1);
  }
  console.log(`✓ ${branch}: ${mode === 'feature' ? `${newKeys.length} FF nova(s), nenhuma duplicada` : `${mode === 'remove' ? `${removedKeys.length} FF(s) a remover` : `${changedKeys.length} FF existente(s) alterada(s)`}`}`);
