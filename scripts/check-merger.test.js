...
function exec(argv, { env = {}, message = 'Merge pull request #12 from drianimadriano/chore/x', mergedBy = 'adrianoo-del', status = 200 } = {}) {
  ...
  const e = { GITHUB_REPOSITORY: 'drianimadriano/porto-feature-flags', GITHUB_TOKEN: 'tok', GITHUB_API_URL: 'https://api.github.com', ...env };
...
test('lê quem mesclou pela API (merged_by do PR do commit de merge) e aprova o admin', async () => {
  ...
  assert.strictEqual(r.calls[0].url, 'https://api.github.com/repos/drianimadriano/porto-feature-flags/pulls/12');
